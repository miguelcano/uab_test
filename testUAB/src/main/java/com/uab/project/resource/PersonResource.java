...
public class PersonResource {
	...
	public ResponseEntity<ResponseDefault> inserPerson(@RequestBody PersonDTO personDto) {
		PersonModel person = personService.validate(personDto, true);
	...
	public ResponseEntity<ResponseDefault> updatePerson(@RequestBody PersonDTO personDto) {
		PersonModel person = personService.validate(personDto, false);
		person = personService.create(person);
		personDto = personService.getDto(person);
		ResponseDefault responseDefault = new ResponseDefault(personDto, ResponseCode.OK.getInternalCode(), "Actualizado.");
		return new ResponseEntity<ResponseDefault>(responseDefault, HttpStatus.valueOf(ResponseCode.OK.getHttpStatus()));
